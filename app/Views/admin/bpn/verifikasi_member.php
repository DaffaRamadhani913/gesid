...
<div class="container-fluid py-4">
    <h4 class="mb-1 fw-bold">Verifikasi Member</h4>
    <p class="text-muted mb-4">Tinjau dan verifikasi data pendaftaran member baru GESID.</p>

    <div class="card shadow-sm border-0">
        <div class="card-body">
            <h5 class="card-title fw-semibold mb-3">Daftar Pendaftaran Member</h5>
                    ...
                    <tbody>
                        <?php foreach ($members as $member): ?>
                            <tr>
                                ...
                                <td class="text-center">
                                    <?php if ($member['status'] !== 'Aktif'): ?>
                                        <a href="<?= base_url('member/activate/' . $member['id']) ?>" class="btn btn-sm btn-success">Aktifkan</a>
                                    <?php endif; ?>
                                    <?php if ($member['status'] !== 'Nonaktif'): ?>
                                        <a href="<?= base_url('member/deactivate/' . $member['id']) ?>" class="btn btn-sm btn-danger">Nonaktifkan</a>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                        <?php if (empty($members)): ?>
                            <tr>
                                <td colspan="14" class="text-center text-muted">Tidak ada data member yang tersedia.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
